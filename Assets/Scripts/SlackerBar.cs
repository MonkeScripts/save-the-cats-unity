...
public class SlackerBar : MonoBehaviour
{
    ...
    [Header("Penalty Flash Text")]
    [SerializeField] private GameObject penaltyTextObject;    // Pre-placed "-5 😿" TextMeshProUGUI in Canvas (starts disabled)
    [SerializeField] private float      flashDuration = 1f;   // How long the text appears
    ...
    void Awake()
    {
        ...
        if (penaltyTextObject != null) penaltyTextObject.SetActive(false);
    ...
    public void EndGame()
    {
        ...
        Debug.Log($"{TAG} 🏁 Game ended. Bar, text, and penalty text hidden.");
    }
    ...
    public void ResetForNewRound()
    {
        ...
        Debug.Log($"{TAG} 🔄 Reset for new round. Bar, text, and penalty text hidden.");
    }
    ...
    private void FlashPenaltyText()
    {
        if (penaltyTextObject == null)
        {
            Debug.LogWarning($"{TAG} ⚠️ penaltyTextObject not assigned!");
            return;
        }

        // If already flashing, restart the timer
        if (flashCoroutine != null)
        {
            StopCoroutine(flashCoroutine);
        }
        ...
        Debug.Log($"{TAG} 💬 Penalty text flashed!");
    }
    ...
    private IEnumerator FlashRoutine()
    {
        ...
        flashCoroutine = null;
        Debug.Log($"{TAG} 💬 Penalty text hidden.");
    }
